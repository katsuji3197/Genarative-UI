...
- 参加者ID
- 実験群/統制群の区別
- 適用されたUI構成（layout, text, button, input, description）
- Presentation設定（アイコン/テキスト表示方法）
- AI判断理由（実験群の場合）
- 事前アンケートの回答（6項目）
- 各画面での滞在時間（ダッシュボード、プロフィール、タスク別）
- タスク完了時間
- 総クリック数
- タスク成功/失敗
- 事後アンケートの回答（4項目）

詳細な仕様は **[実験データ仕様書](./EXPERIMENT_DATA_SPECIFICATION.md)** を参照してください。
